...
const TRUST_DAYS  = 30
const STORAGE_KEY = 'dtokens' // JSON map  hashedEmail → { fingerprint, expiresAt }

// ── Helpers ───────────────────────────────────────────────────────────────────

/** djb2 hash — fast, dependency-free hash for storage keys. */
function djb2 (str) {
  let hash = 5381
...
/** Hash email so plain-text PII is never stored as a localStorage key. */
function hashEmail (email) {
  return djb2(email.toLowerCase())
}
...
export function getTrustToken (email) {
  ...
  try {
    const record = readAll()[hashEmail(email)]
